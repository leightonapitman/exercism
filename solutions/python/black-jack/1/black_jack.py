...
def value_of_card(card: str) -> int:
    ...
    valid_numeric_cards = [str(n) for n in range(2, 11)]

    if card in valid_numeric_cards:
        return int(card)
    elif card in ['J', 'Q', 'K']:
        return 10
    elif card == 'A':
...
def value_of_ace(card_one: str, card_two: str) -> int:
    ...
    ace_is_present: bool = (card_one == 'A') or (card_two == 'A')
...
def is_blackjack(card_one: str, card_two: str) -> bool:
    ...
    if card_one == 'A' or card_two == 'A':
        hand_value += 10  # Count ace as 11 instead of 1
